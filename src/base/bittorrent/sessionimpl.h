...
namespace BitTorrent
{
    ...
    public:
        ...
        {
            // Wrap in shared_ptr to handle move-only functors
            auto task = std::make_shared<std::function<void()>>([func = std::forward<Func>(func)]() mutable { func(); });
            m_asyncWorker->start(*task);
        }
