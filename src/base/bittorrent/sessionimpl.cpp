...
// qHash specialization for boost::asio::ip::basic_endpoint<tcp> used in QHash
namespace boost { namespace asio { namespace ip {
inline std::size_t qHash(const boost::asio::ip::basic_endpoint<boost::asio::ip::tcp> &endpoint, std::size_t seed = 0) noexcept
{
    return qHashMulti(seed, endpoint.address().to_string().c_str(), endpoint.port());
}
}}}
#include <algorithm>
#include <concepts>
...
void SessionImpl::endStartup(ResumeSessionContext *context)
{
    ...
    connect(context, &QObject::destroyed, this, [this]
    {
        ...
        {
            const bool hasSystemSlept = (std::chrono::seconds)m_wakeupCheckTimestamp.elapsed() > std::chrono::seconds(100);
            if (hasSystemSlept)
            {
...
lt::settings_pack SessionImpl::loadLTSettings() const
{
    ...
    // I2P session options - removed for libtorrent 2.0.x compatibility
